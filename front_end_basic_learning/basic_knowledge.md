...
- Next.js项目构建完成后得到的`.next`项目并非可直接部署的静态文件包，而是一个专为 Node.js 服务器环境优化的、可执行的“服务端应用包”
  - 其中可能包含了动态渲染逻辑（如服务端渲染、API路由），该逻辑需Node.js环境实时执行。因此出于通用性，Next.js项目构建完成后默认需要`next start`启动服务。**（常态服务：需要服务器中常驻一个不休止的Node进程）**
    - 进一步深挖便是Next全栈实现：服务器组件可以直接在服务器上运行，因此可以直接连接数据库、获取数据，同时使用服务器才使用的密钥，因此可以在Next向内直接写后端接口。
...
- 注意到路由管理是由URL进行，但静态构建后部署到服务器时一般只能渲染入口页面，无法渲染其他页面。因此在静态导出后，我们再nginx上需要设置`try_files $uri $uri.html $uri/ = 404;`，这样当用户访问其他页面时，nginx会自动将请求转发到入口页面，然后入口页面会根据URL中的参数动态加载其他页面内容。这样就实现了静态导出后也能实现动态路由。